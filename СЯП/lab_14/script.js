import { GRID_SIZE, BOX_SIZE, convertPositionToIndex, convertIndexToPosition } from "./constates.js";
import { Sudoku } from "./sudoku.js";
import { answerSudoku } from "./generatorSudoku.js";
...
function setValueInSelectedCell(value) {
  ...
  // Удаление класса answer, если значение введено верно
  if (value === answerSudoku[row][column]) {
    selectedCell.classList.remove('answer');
  }
}
...
function showAnswers() {
  for (let i = 0; i < GRID_SIZE * GRID_SIZE; i++) {
    ...
    if (!cells[i].classList.contains('filled')) {
      cells[i].innerHTML = answerSudoku[row][column];
      cells[i].classList.toggle('answer');
    }
  }
}

// кнопка очищения пользовательского ввода
...
function clearUserInput() {
  for (let i = 0; i < GRID_SIZE * GRID_SIZE; i++) {
    ...
    if (!cells[i].classList.contains('filled')) {
      cells[i].innerHTML = '';
      sudoku.grid[row][column] = null;
      cells[i].classList.remove('answer');
    }
  }
}

// кнопка генерации нового поля судоку
