import { BOX_SIZE, GRID_SIZE } from "./constates.js";
export let answerSudoku;

export function generatorSudoku() {
    const sudoku = crateEmptyGrid();
    resolveSudoku(sudoku);
    return removeCells(sudoku);
}

function crateEmptyGrid() {
...
function validateColumn(grid, row, column, value) {
    for (let iRow = 0; iRow < GRID_SIZE; iRow++) {
        if (grid[iRow][column] === value && iRow !== row)
            return false;   
    }
...
function validateRow(grid, row, column, value) {
    for (let iColumn = 0; iColumn < GRID_SIZE; iColumn++) {
        if (grid[row][iColumn] === value && iColumn !== column)
            return false;
    }
...
function removeCells(grid) {
    const DIFFUCULTY = 10;  // кол-во удаляемых ячеек
    const resultGrid = [...grid].map(row => [...row]);  // копируем sudoku
    answerSudoku = grid;  // сохраняем правильный судоку
